...
def prep_personality_data(
    ...
    genre_movie_df = genre_movie_df.rename(
        columns={"movieId": "movie_id", "genreId": "genre_id"}
    )
    ...
    genre_movie_df = genre_movie_df.drop_duplicates(
        subset=["movie_id", "genre_id"], keep="last"
    )
...
def main():
    ...
    data = load_csv(base_dirs["raw"], "data.csv", delimiter=", ")
    ratings = load_csv(base_dirs["raw"], "ratings.csv", delimiter=", ")
    movies = load_csv(base_dirs["raw"], "movies.csv")
    users_df, movies_df, genres_df, genre_movie_df, ratings_df = prep_personality_data(
        data, movies, ratings
    )
