...
def process_and_normalise_tags(tags_df, movies_df, users_df):
    ...
    tags_with_ids_df = tags_with_ids_df.merge(normalized_tags_df, on="tag", how="left")

    movies_users_tags_df = tags_with_ids_df[["movieId", "userId", "tagId", "timestamp"]]
...
def prep_personality_data(
    data: pd.DataFrame, movies: pd.DataFrame, ratings: pd.DataFrame
) -> tuple:
    users = data[
        [
            "userid",
    ...
    ratings_df = valid_ratings.loc[
        valid_ratings["userId"].isin(users["userid"].unique())
    ]
...
def main():
    ...
    links_df = load_csv(base_dirs["raw"], "links.csv", dtype={"tmdbId": "str"})
    movies_df = load_csv(base_dirs["raw"], "movies.csv")
    ratings_df = load_csv(base_dirs["raw"], "ratings.csv")
    tags_df = load_csv(base_dirs["raw"], "tags.csv")
