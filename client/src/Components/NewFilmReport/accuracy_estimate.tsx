...
const AccuracyEstimate = () => {
    const [estimated_accuracy, setEstimatedAccuracy] = useState<EstimatedAccuracyType[] | null>(null);
    ...
    async function handleSubmit() {
        const response = await fetch('http://localhost:5555/movies/get-search-results', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const estimated_ratings_promises = movie_names.map((name: string) => getMoviePrediction(name));
        const estimated_ratings = await Promise.all(estimated_ratings_promises);

        const estimated_accuracy = movie_names.map((name: string, index: number) => ({
            name: name,
            rating: movie_ratings[index],
            estimated_rating: estimated_ratings[index].predictedRating,
        }));

        const total_difference = estimated_accuracy.reduce((acc: any, movie: any) => acc + Math.abs(movie.rating - movie.estimated_rating), 0);
